...
export function NewVideoList({ channelUrl }: NewVideoListProps) {
  ...
  return (
    <div
      className={css({ p: '1rem', border: '1px solid #eee', borderRadius: '8px', width: '400px' })}
    >
      ...
      {!channelUrl ? (
        ...
        <>
          <p className={css({ mb: '1rem' })}>
            채널: <span className={css({ fontWeight: 'bold' })}>{channelUrl}</span>
          </p>
          {loading ? (
            <p>새 동영상을 불러오는 중...</p>
          ) : videos.length === 0 ? (
              ...
              {videos.map((video) => (
                ...
                >
                  <div className={css({ flexGrow: 1 })}>
                    ...
                    <p className={css({ fontSize: '0.8rem', color: '#666' })}>
                      {video.publishedAt}
                    </p>
                  </div>
                  <button
